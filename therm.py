...
face_cascade = cv2.CascadeClassifier(f'{base_dir}/haarcascade_frontalface_default.xml')
...
display_temp = 98.6
room_temp = 65.0
base_img_dir = base_dir.joinpath('static', 'img')
og_frame = cv2.imread(f"{base_img_dir}/therm_background.png")
blank_screen = cv2.imread(f"{base_img_dir}/default2.png")
wait_ = cv2.imread(f"{base_img_dir}/clock.png")
stop = cv2.imread(f"{base_img_dir}/stop.png")
go = cv2.imread(f"{base_img_dir}/go.png")
...
while True:
    ...
    pixels = np.random.normal(36.805127778, 0.4, 64)
